...
final class SearchViewModel {
    ...
    private func transform() {
        ...
        // 단일 검색어 삭제 버튼
        inputRemoveSearchList.bind { [weak self] tag in
        ...
        // 전체 검색어 삭제 버튼
        inputRemoveAllSearchList.bind { [weak self] _ in
            UserDefaultsManager.search.removeAll()
            self?.outputSearchList.value = UserDefaultsManager.search
        }
        
        // 상품 검색
        inputSearchButtonClicked.bind { [weak self] searchText in
        ...
        // 기존 검색어 리스트 클릭
        inputSearchListClicked.bind { [weak self] idx in
